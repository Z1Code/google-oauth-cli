...
export async function fillBranding(
  ...
  const targets = [
    [/p[aá]gina principal|home ?page|application home/i, homepage, "homepage"],
    [/pol[ií]tica de privacidad|privacy policy/i, privacy, "privacy"],
    [/condiciones del servicio|terms of service/i, terms, "terms"],
    [/direcciones de correo|developer contact|email addresses/i, contact, "contact"],
  ];

  const written = [];
  for (const [pattern, value, name] of targets) {
    if (!value) continue;
    const r = await setField(page, pattern, value);
  ...
  const persisted = targets
    ...
    .map(([, value, name]) => ({
      field: name,
      value,
      persisted: after.some((f) => f.value === value),
    }));
